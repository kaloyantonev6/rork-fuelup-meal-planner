...
final class BudgetStore {
    private enum Keys {
        static let budget = "fuelify_weekly_budget_v1"
        static let purchases = "fuelify_purchases_v1"
        // Pre-rebrand keys — still read so existing installs keep their data.
        static let legacyBudget = "fuelup_weekly_budget_v1"
        static let legacyPurchases = "fuelup_purchases_v1"
    }
    ...
    init(defaults: UserDefaults = .standard, fallbackBudget: Double = 35) {
        ...
        let stored = defaults.double(forKey: Keys.budget) != 0
            ? defaults.double(forKey: Keys.budget)
            : defaults.double(forKey: Keys.legacyBudget)
        self.weeklyBudget = stored > 0 ? stored : fallbackBudget

        if let data = defaults.data(forKey: Keys.purchases) ?? defaults.data(forKey: Keys.legacyPurchases),
