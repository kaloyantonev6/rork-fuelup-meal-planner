...
final class DayProgressStore {
    private enum Keys {
        static let completedPrefix = "fuelify_completed_sessions_"
        static let hydration = "fuelify_hydration_v1"
        // Pre-rebrand keys — still read so existing installs keep their data.
        static let legacyCompletedPrefix = "fuelup_completed_sessions_"
        static let legacyHydration = "fuelup_hydration_v1"
    }
    ...
    private func load() {
        ...
        } else if let stored = defaults.array(forKey: Keys.legacyCompletedPrefix + today) as? [Int] {
            completedIndices = Set(stored)
        } else {
            completedIndices = []
        }

        if let record = defaults.dictionary(forKey: Keys.hydration) ?? defaults.dictionary(forKey: Keys.legacyHydration),
